...
//allowed extentions
$GLOBALS['registry']->allowedImageExtensions = array('png', 'gif', 'jpg', 'jpeg');
...
class Image {
    ...
    function Image($image_file) {
        ...
        switch ($img['mime']) {
            case 'image/png' : $image = imagecreatefrompng($image_file);
                break;
            case 'image/jpeg': $image = imagecreatefromjpeg($image_file);
                break;
            case 'image/gif' :
...
//Create the image
    ...
    function save($file_name, $destroy = true) {
        ...
        switch ($extension) {
            case 'png' : return imagepng($this->image, $file_name);
                break;
            case 'jpeg':
            case 'jpg' : return imagejpeg($this->image, $file_name);
                break;
            case 'gif' : return imagegif($this->image, $file_name);
                break;
            default: break;
        }
        if ($destroy)
...
/* function doImage($imagePath,$imageName){
  $myImage =  new image($imagePath);
  $i=0;
  //var_dump($GLOBALS['registry']->uploadImagesOptions['thumbsTypes'] );
  foreach($GLOBALS['registry']->uploadImagesOptions['thumbsTypes'] as $type){
  $myImage->resize($GLOBALS['registry']->uploadImagesOptions['width'][$type],$GLOBALS['registry']->uploadImagesOptions['height'][$type],false);
  $myImage->save($GLOBALS['registry']->uploadImagesOptions['path'][$type].$imageName);
  /*		if($type==POST_THUMB || $type==SLIDER_THUMB )
  watermark($GLOBALS['registry']->uploadImagesOptions['path'][$type].$imageName,$GLOBALS['sadaLogo'],$GLOBALS['registry']->uploadImagesOptions['path'][$type].$imageName);
  if($i==sizeof($GLOBALS['registry']->uploadImagesOptions['thumbsTypes'])-1)
  $myImage->destroy();
  else
  $myImage->restore();
  $i++;
  ...
  if (file_exists ( REAL_IMAGE_PATH.$imageName ))
  unlink ( REAL_IMAGE_PATH.$imageName);

  foreach($GLOBALS['registry']->uploadImagesOptions['thumbsTypes'] as $type)
  if (file_exists ( $GLOBALS['registry']->uploadImagesOptions['path'][$type].$imageName ))
  unlink ( $GLOBALS['registry']->uploadImagesOptions['path'][$type].$imageName);
  }
 */

function doImage($imagePath, $imageName, $imagePathsKeys, $uploadImagesOptions, $destroyRealImage = true) {
    $myImage = new image($imagePath);
    $i = 0;

    foreach ($imagePathsKeys as $imagePathKey) {
        $myImage->resize($uploadImagesOptions[$imagePathKey]['width'], $uploadImagesOptions[$imagePathKey]['height'], false);
        $myImage->save($uploadImagesOptions[$imagePathKey]['path'] . $imageName);
    ...
    if ($destroyRealImage and file_exists($imagePath))
        unlink($imagePath);
    return $imageName;
}

function deleteImages($imageName, $imagePathsKeys, $uploadImagesOptions) {

    foreach ($imagePathsKeys as $imagePathKey) {
        if (file_exists($uploadImagesOptions[$imagePathKey]['path'] . $imageName))
            unlink($uploadImagesOptions[$imagePathKey]['path'] . $imageName);
    }
}
